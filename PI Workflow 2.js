...
#feature-id    Utilities > PI_Workflow_Opt_6d
...
OptPreviewPane.prototype.storeMemory = function(index) {
   ...
   if (optSafeView(view)) {
      // Pre-flight: warn the user if this store will push usage over budget.
      var addedBytes = optEstimateViewBytes(view) + optEstimateViewBytes(gradientView);
      try { optMemoryPreflight(this.dialog, addedBytes, "image memory store"); } catch (ePF) {}
      this.memory.store(index, this.currentKey || view.id, view, meta, gradientView);
      // Smart budget enforcement: the just-stored slot is ALWAYS protected so the
      // user's explicit save can never be evicted. Older non-recalled slots in other
      // tabs/managers are released if total usage exceeds OPT_MEMORY_BUDGET_BYTES.
      try {
         optEnforceMemoryBudget(this.dialog, "image memory store",
            { manager: this.memory, index: index });
      } catch (eMB) {}
   }
};
...
function optApplyProcessAvailabilityToUI(dlg) {
   ...
   var hasDBE  = OPT_PIW_HAS_AUTODBE;
...
function optBuildWorkflowTitleBar(parent) {
   ...
   helpButton.onClick = function() {
      ...
      try { var ep = new ExternalProcess(); ep.start("cmd", ["/c", "start", "", helpPath]); } catch (e) {}
   };
