...
OptWorkflowTab.prototype.combineNb = function() {
   ...
   if (optSafeView(hoView) && optSafeView(soView)) {
      ...
      try {
         ...
         var combinedDbx;
         if (palette === "GOLDEN") {
            // GOLDEN-PALETTE: weighted tint blend, not a 1:1 mapping.
            combinedDbx = optCreateGoldenNbFromChannels(extracted.ha, extracted.oiii, extracted.sii, "NB_RGB_DBX_GOLDEN", extracted.oiii);
         } else {
            var recipeDbx = optRecipeChannels(palette);
            var rD = mapDbx[recipeDbx[0]];
            var gD = mapDbx[recipeDbx[1]];
            var bD = mapDbx[recipeDbx[2]];
            combinedDbx = optCreateRgbFromChannels(rD, gD, bD, "NB_RGB_DBX_" + palette, gD || rD || bD);
         }
   ...
   var combined;
   if (String(this.dialog.selectedRecipe || "").toUpperCase() === "GOLDEN") {
      // GOLDEN-PALETTE: weighted tint blend (S optional), not a 1:1 mapping.
      combined = optCreateGoldenNbFromChannels(map.H, map.O, map.S, "NB_RGB_GOLDEN", map.O || map.H);
   } else {
      var recipe = optRecipeChannels(this.dialog.selectedRecipe);
      var r = map[recipe[0]];
      var g = map[recipe[1]];
      var b = map[recipe[2]];
      combined = optCreateRgbFromChannels(r, g, b, "NB_RGB_" + this.dialog.selectedRecipe, g || r || b);
   }
