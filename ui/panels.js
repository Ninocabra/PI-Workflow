...
// LOAD-AUTOASSIGN: programmatic selection by view id (slot auto-assignment
// after "Load Image Files…"). Setting currentItem from code does NOT emit
// onItemSelected, so the shared onSelectionChanged callback is invoked here
// explicitly — dependent UI (e.g. the L-weight slider auto-show) must react
// the same as to a manual pick. Returns false when the view is not in this
// combo's list (wrong mono/color type for the slot, closed window, …).
OptImageCombo.prototype.selectViewById = function(viewId) {
   for (var i = 0; i < this.views.length; ++i) {
      if (this.views[i].id !== viewId) continue;
      try { this.combo.currentItem = i + 1; } catch (e) { return false; }
      if (typeof this.onSelectionChanged === "function")
         this.onSelectionChanged(this.selectedView());
      return true;
   }
   return false;
};
...
function OptSelectionPanel(dialog, tab) {
   ...
   this.btnLoadFiles.onClick = function() {
      optSafeUi("Load Image Files", function() {
         ...
         if (!ofd.execute())
            return;
         var opened = 0, openedEntries = [];
         for (var i = 0; i < ofd.fileNames.length; ++i) {
            try {
               var wins = ImageWindow.open(ofd.fileNames[i]);
               for (var k = 0; k < wins.length; ++k) {
                  try { wins[k].show(); } catch (eS) {}
                  ++opened;
                  // LOAD-AUTOASSIGN: remember what was opened so the still-empty
                  // slots of the active mode can be seeded below. Multi-image XISF
                  // (WBPP masters + rejection maps) push several entries with the
                  // same fileName — the master comes first and wins the slot.
                  try {
                     openedEntries.push({
                        viewId: wins[k].mainView.id,
                        fileName: ofd.fileNames[i],
                        filter: optReadFilterKeyword(wins[k])
                     });
                  } catch (eE) {}
               }
            } catch (eO) {
         ...
         panelSelf.refresh();
         var assigned = panelSelf.autoAssignSlots(openedEntries);
         for (var a = 0; a < assigned.length; ++a)
            console.writeln("=> Load Image Files: auto-assigned " + assigned[a]);
...
OptSelectionPanel.prototype.buildNbGroup = function() {
   ...
   // Build 31: 4th row for palettes beyond the 12 classic ones (GOLDEN, ...).
   // GOLDEN lands at row 4 col 1 — directly below HSS, as requested.
   var recipeRow4 = new Control(this.recipeRow);
   recipeRow4.sizer = new HorizontalSizer();
   recipeRow4.sizer.spacing = 3;
   for (var i = 0; i < OPT_RECIPE_NAMES.length; ++i) {
      var recipeParent = i < 4 ? recipeRow1 : (i < 8 ? recipeRow2 : (i < 12 ? recipeRow3 : recipeRow4));
   ...
   // Build 31: pad row 4 so its buttons keep the same 1/4-row width as the
   // other rows (GOLDEN sits in column 1, aligned under HSS).
   var row4Buttons = OPT_RECIPE_NAMES.length - 12;
   if (row4Buttons > 0 && row4Buttons < 4)
      recipeRow4.sizer.addStretch(4 - row4Buttons);
   this.recipeRow.sizer.add(recipeRow1);
   this.recipeRow.sizer.add(recipeRow2);
   this.recipeRow.sizer.add(recipeRow3);
   if (row4Buttons > 0)
      this.recipeRow.sizer.add(recipeRow4);
...
OptSelectionPanel.prototype.setMode = function(mode) {
   ...
   // LOAD-AUTOASSIGN: entering a mode seeds ITS still-empty slots (the images
   // may have been opened while another mode was active). Harmless during
   // construction: the combos are empty until the first refresh(), so
   // selectViewById simply finds nothing.
   this.autoAssignFromOpenWindows();
};
...
OptSelectionPanel.prototype.refresh = function() {
   ...
   // LOAD-AUTOASSIGN: a refresh rebuilds every combo (dropping any selection
   // that is not an exact id==key match), so re-seed the empty slots of the
   // active mode from whatever is already open in the workspace.
   this.autoAssignFromOpenWindows();
};
...
// LOAD-AUTOASSIGN: after "Load Image Files…" refreshes the combos, place each
// opened image in the matching slot of the ACTIVE mode, inferred from the
// file-name suffix (_HO/_OS/_Ha/…) and, failing that, the FILTER keyword.
// A slot is only filled while it still reads "None" — user selections are
// never overwritten and the first match wins when several files claim the
// same slot. entries: [{viewId, fileName, filter}] → ["H <- id (name)", …].
OptSelectionPanel.prototype.autoAssignSlots = function(entries) {
   var slotToCombo = {
      MONO: { R: "R", G: "G", B: "B", L: "L_MONO" },
      NB:   { H: "H", O: "O", S: "S", HO: "HO", OS: "OS" },
      RGB:  { RGB: "RGB" }
   }[this.mode] || {};
   var assigned = [];
   for (var i = 0; i < entries.length; ++i) {
      var e = entries[i] || {};
      var source = "name";
      var slot = optInferSlotFromName(e.fileName);
      if (!slot && e.filter) { slot = optInferSlotFromFilter(e.filter); source = "FILTER"; }
      if (!slot) continue;
      var combo = this.combos[slotToCombo[slot]];
      if (!combo) continue;                          // slot ajeno al modo activo
      var cur = 0;
      try { cur = combo.combo.currentItem; } catch (eC) { cur = 0; }
      if (cur > 0) continue;                         // slot ya ocupado: no pisar
      if (combo.selectViewById(e.viewId))
         assigned.push(slot + " <- " + e.viewId + " (" + source + ")");
   }
   return assigned;
};

// LOAD-AUTOASSIGN (build 34): seed slots from windows that are ALREADY open.
// Covers the flow the button cannot see (user report 2026-07-08): the masters
// were opened with File > Open / drag & drop BEFORE launching the script, so
// MasterLight_H/O/S stayed in "None". Sources per window: the on-disk file
// path (win.filePath) — falling back to the view id for in-session views —
// plus the FILTER keyword. Runs after every combo refresh and on mode change;
// the only-if-None contract of autoAssignSlots keeps user selections safe.
OptSelectionPanel.prototype.autoAssignFromOpenWindows = function() {
   var entries = [];
   try {
      var windows = ImageWindow.windows;
      for (var i = 0; i < windows.length; ++i) {
         var view = windows[i].mainView;
         if (!optSafeView(view)) continue;
         var path = "";
         try { path = windows[i].filePath; } catch (eP) {}
         entries.push({
            viewId: view.id,
            fileName: (path && path.length > 0) ? path : view.id,
            filter: optReadFilterKeyword(windows[i])
         });
      }
   } catch (e) {}
   var assigned = this.autoAssignSlots(entries);
   for (var a = 0; a < assigned.length; ++a)
      console.writeln("=> Image Selection: auto-assigned " + assigned[a]);
   return assigned;
};
