...
#engine v8
#feature-id    Utilities > PI_Workflow_194_2
#feature-info  PI Workflow for PixInsight 1.9.4 V8 - live-preview downsampling, pre-process CC caching, row-buffer MAS sampling, and reusable preview paint buffers.
...
#define WCS_DEFAULT_RBF_TYPE RadialBasisFunction.DDMThinPlateSpline
#endif
...
var OPT_VERSION = "194_2-v8-performance-rc1";
...
var OPT_MOUSE_LEFT = (typeof MouseButton !== "undefined" && typeof MouseButton.Left !== "undefined") ? MouseButton.Left : 0x01;
var OPT_MOUSE_RIGHT = (typeof MouseButton !== "undefined" && typeof MouseButton.Right !== "undefined") ? MouseButton.Right : 0x02;
...
function optTryLoadVeraLuxScript(candidatePaths, quiet) {
   ...
   if (existing < 1) {
      var sample = [];
      for (var j = 0; j < FMath.min(12, candidatePaths.length); ++j)
...
function optClampPreviewReduction(value) {
   ...
   return FMath.max(1, FMath.min(6, v));
}
...
function optMadMidtone(median, shadow, targetBackground) {
   ...
   if (FMath.abs(denom) < 1.0e-12)
      return 0.5;
   var midtone = (target - 1) * value / denom;
   if (!isFinite(midtone))
      return 0.5;
   return FMath.max(0.0001, FMath.min(0.9999, midtone));
}
...
function optApplyMadAutoStretch(image, linked) {
   ...
   try {
      if (linked === true && channels >= 3) {
         ...
         var linkedShadow = FMath.max(0.0, linkedMedian - 1.25 * linkedMad);
...
function optRenderPreviewBitmap(view, reductionFactor, stretchMode) {
   ...
   try {
      ...
      var rw = FMath.max(1, FMath.round(w / reduction));
      var rh = FMath.max(1, FMath.round(h / reduction));
      var previewInterpolation = InterpolationAlgorithm.Bilinear;
      try {
         var viewId0 = String(view.id || "");
         if (view.image.numberOfChannels === 1 &&
             (viewId0.indexOf("RangeMaskBinary") >= 0 || viewId0.indexOf("RS-BIN") >= 0))
            previewInterpolation = InterpolationAlgorithm.NearestNeighbor;
      } catch (eI0) {}
      img = new Image(w, h, view.image.numberOfChannels, view.image.colorSpace, 32, PixelSampleType.Real);
...
function optRenderPreviewBitmapToSize(view, targetW, targetH, stretchMode) {
   ...
   try {
      if (!view.image)
         return null;
      var rw = FMath.max(1, targetW);
      var rh = FMath.max(1, targetH);
      var previewInterpolation = InterpolationAlgorithm.Bilinear;
      try {
      ...
      img = new Image(view.image.width, view.image.height, view.image.numberOfChannels, view.image.colorSpace, 32, PixelSampleType.Real);
      img.assign(view.image);
...
function optBuildPreviewImage(view, targetW, targetH, stretchMode) {
   ...
   var img = new Image(view.image.width, view.image.height, view.image.numberOfChannels, view.image.colorSpace, 32, PixelSampleType.Real);
   img.assign(view.image);
   if (rw !== view.image.width || rh !== view.image.height)
      img.resample(rw, rh, previewInterpolation);
   if (stretchMode === "mad-unlinked")
...
function optRenderPreviewBitmapWithMask(view, maskView, reductionFactor, stretchMode) {
   ...
   if (view.image.width !== maskView.image.width || view.image.height !== maskView.image.height)
      return null;
   var reduction = optClampPreviewReduction(reductionFactor);
   var rw = FMath.max(1, FMath.round(view.image.width / reduction));
   var rh = FMath.max(1, FMath.round(view.image.height / reduction));
   var srcImg = null;
   var maskImg = null;
   try {
      ...
      for (var y = 0; y < rh; ++y) {
         ...
         for (var x = 0; x < rw; ++x) {
            ...
            var rr = FMath.max(0, FMath.min(255, FMath.round(255 * (rv * (1 - a) + a))));
            var gg = FMath.max(0, FMath.min(255, FMath.round(255 * (gv * (1 - a)))));
            var bb = FMath.max(0, FMath.min(255, FMath.round(255 * (bv * (1 - a)))));
...
function optRenderStackedPreviewBitmap(topView, bottomView, reductionFactor, stretchMode) {
   ...
   var targetW = FMath.max(1, FMath.round(top.width / 2));
   var targetH = FMath.max(1, FMath.round(top.height / 2));
   var bottom = optRenderPreviewBitmapToSize(bottomView, targetW, targetH, "mad-unlinked");
   if (!bottom)
      return top;
   var gap = 8;
   var out = new Bitmap(FMath.max(top.width, bottom.width), top.height + gap + bottom.height);
   var g = new Graphics(out);
   try {
      g.fillRect(new Rect(0, 0, out.width, out.height), new Brush(0xff202020));
      g.drawBitmap(FMath.round((out.width - top.width) / 2), 0, top);
      g.fillRect(new Rect(0, top.height, out.width, top.height + gap), new Brush(0xff0e0e10));
      g.drawBitmap(FMath.round((out.width - bottom.width) / 2), top.height + gap, bottom);
   } finally {
      try { g.end(); } catch (eG) {}
      try { top.clear(); } catch (eTop) {}
      try { bottom.clear(); } catch (eBottom) {}
   }
   return out;
}
...
function optApplyLuminanceLRGB(rgbView, luminanceView) {
   ...
   var inProcess = false;
   try {
      rgbView.beginProcess(UndoFlag.NoSwapFile);
      inProcess = true;
...
function optUiError(title, error) {
   ...
   try { new MessageBox(title + ": " + message, "PI Workflow", StdIcon.Error, StdButton.Ok).execute(); } catch (e1) {}
}
...
function optProcessValuesEquivalent(a, b) {
   ...
   try {
      ...
      if (isFinite(na) && isFinite(nb))
         return FMath.abs(na - nb) <= 1.5e-12;
   } catch (e1) {}
   try {
...
function optExtractGrayChannelView(sourceView, channelIndex, baseId) {
   ...
   var gray = new Image(w, h, 1, ColorSpace.Gray, 32, PixelSampleType.Real);
   srcImg.selectedChannel = channelIndex;
   gray.selectedChannel = 0;
   gray.apply(srcImg, ImageOp.Mov);
...
function optApplyFallbackTransform(view, family, strength) {
   var s = FMath.max(0, FMath.min(1, isFinite(strength) ? strength : 0.15));
   if (family === "stretch") {
      var gamma = FMath.max(0.25, 1.0 - 0.55 * s);
...
function optClamp01(v) {
   ...
   return FMath.max(0.0, FMath.min(1.0, x));
}
...
function optGenerateHueWheelBitmap(size, innerRatio, northZero) {
   var sz = FMath.max(32, FMath.round(size || 160));
   var img = new Image(sz, sz, 3, ColorSpace.RGB, 32, PixelSampleType.Real);
   var cx = (sz - 1) * 0.5;
   var cy = (sz - 1) * 0.5;
   var outer = FMath.max(1.0, sz * 0.5 - 1.0);
   var inner = FMath.max(0.0, FMath.min(0.95, innerRatio || 0.0)) * outer;
   try {
      for (var y = 0; y < sz; ++y) {
         for (var x = 0; x < sz; ++x) {
            var dx = x - cx;
            var dy = y - cy;
            var r = FMath.sqrt(dx * dx + dy * dy);
            if (r > outer || r < inner) {
            ...
            var hue = (northZero === true ? FMath.atan2(dx, -dy) : FMath.atan2(dy, dx)) / (2.0 * FMath.PI);
            if (hue < 0.0)
               hue += 1.0;
            var sat = optClamp01((r - inner) / FMath.max(1.0e-6, outer - inner));
...
function optComputeViewMeanHueSat(view, maxSamples) {
   ...
   var img = view.image;
   var step = FMath.max(1, FMath.ceil(FMath.sqrt((img.width * img.height) / FMath.max(128, maxSamples || 4096))));
   ...
   for (var y = 0; y < img.height; y += step) {
      for (var x = 0; x < img.width; x += step) {
         ...
         var w = FMath.max(0.02, hs.sat);
         var a = hs.hue * 2.0 * FMath.PI;
         sumSin += FMath.sin(a) * w;
         sumCos += FMath.cos(a) * w;
         sumSat += hs.sat * w;
         sumWeight += w;
      }
   }
   if (sumWeight <= 0.0)
      return { hueDeg: 0.0, sat: 0.0 };
   var hue = FMath.atan2(sumSin / sumWeight, sumCos / sumWeight) * 180.0 / FMath.PI;
   if (hue < 0.0)
      hue += 360.0;
...
function optComputeHistogramDataForView(view, binsParam) {
   ...
   var bandRows = FMath.max(1, FMath.min(H, FMath.floor(maxBandPixels / FMath.max(1, W))));
   ...
   for (var y0 = 0; y0 < H; y0 += bandRows) {
      var rows = FMath.min(bandRows, H - y0);
      var n = W * rows;
...
OptPreviewScheduler.prototype.request = function(key, fn, options) {
   ...
   var delayMs = FMath.max(0, FMath.round((options && options.debounceMs) || 0));
   var scheduler = this;
   function runNow() {
   ...
   try { timer.interval = FMath.max(0.001, delayMs / 1000.0); } catch (e3) {}
...
function optSolveAstrometryOnWindow(window, contextLabel) {
   ...
   try { solver.solverCfg.rbfType = RadialBasisFunction.DDMThinPlateSpline; } catch (e1) {}
...
function optConfigureABEInstance(abe, dlg, forceModelOutput, forceReplaceTarget) {
   ...
   try {
      if (typeof AutomaticBackgroundExtractor !== "undefined") {
         if (typeof AutomaticBackgroundExtractor.Subtract !== "undefined")
            subtractValue = AutomaticBackgroundExtractor.Subtract;
         if (typeof AutomaticBackgroundExtractor.Divide !== "undefined")
            divideValue = AutomaticBackgroundExtractor.Divide;
      }
   } catch (e0) {}
   var targetCorrectionValue = (correctionIndex === 1) ? divideValue : subtractValue;
   var functionDegree = 1;
   if (dlg && dlg.ncAbeFunctionDegree)
      functionDegree = FMath.max(0, FMath.min(8, FMath.round(dlg.ncAbeFunctionDegree.value)));
   var normalize = false;
   ...
   try {
      if (typeof AutomaticBackgroundExtractor !== "undefined" &&
          typeof AutomaticBackgroundExtractor.SameAsTarget !== "undefined")
         abe.correctedImageSampleFormat = AutomaticBackgroundExtractor.SameAsTarget;
   } catch (e5) {}
...
function optRunAutoDBEGradientCorrection(targetView, params) {
   ...
   if (isMono) {
      ...
      var tmpImg = new Image(w, h, 3, ColorSpace.RGB, 32, PixelSampleType.Real);
      try {
         monoImg.selectedChannel = 0;
         for (var c = 0; c < 3; ++c) {
            tmpImg.selectedChannel = c;
            tmpImg.apply(monoImg, ImageOp.Mov);
         }
         monoImg.resetSelections();
         tmpImg.resetSelections();
         tempWin.mainView.beginProcess(UndoFlag.NoSwapFile);
   ...
   try {
      ...
      if (isMono) {
         var corrImg = new Image(workView.image.width, workView.image.height, 1, ColorSpace.Gray, 32, PixelSampleType.Real);
         try {
            workView.image.selectedChannel = 0;
            corrImg.selectedChannel = 0;
            corrImg.apply(workView.image, ImageOp.Mov);
...
function optConfigureGraXpertNativeProcess(P, mode, dlg) {
   ...
   smoothing = FMath.max(0.0, FMath.min(1.0, smoothing));

   var strength = 1.00;
   try { strength = dlg.ncPostGraXpertStrength.value; } catch (e1) {}
   if (!isFinite(strength))
      strength = 1.00;
   strength = FMath.max(0.0, FMath.min(2.0, strength));

   var batchSize = 4;
   try { batchSize = FMath.round(dlg.ncPostGraXpertBatchSize.value); } catch (e2) {}
   if (!isFinite(batchSize) || batchSize < 1)
      batchSize = 4;
   batchSize = FMath.max(1, FMath.min(16, batchSize));
...
function optRunGraXpertWorkflow(targetView, dlg) {
   ...
   if (!isFinite(smoothing))
      smoothing = OPT_GRAXPERT_DEFAULT_SMOOTHING;
   smoothing = FMath.max(0, FMath.min(1, smoothing));
   var isMono = (targetView.image.numberOfChannels < 3);
   var workView = targetView;
   var gxTempWin = null;
   if (isMono) {
      ...
      var tmpImg = new Image(gxW, gxH, 3, ColorSpace.RGB, 32, PixelSampleType.Real);
      try {
         monoImg.selectedChannel = 0;
         for (var c = 0; c < 3; ++c) {
            tmpImg.selectedChannel = c;
         ...
         monoImg.resetSelections();
         tmpImg.resetSelections();
         gxTempWin.mainView.beginProcess(UndoFlag.NoSwapFile);
   ...
   if (ok === false)
      throw new Error("[GRAXPERT/EXECUTION] GraXpert returned false before completing the process.");
   if (isMono) {
      var corrImg = new Image(gxW, gxH, 1, ColorSpace.Gray, 32, PixelSampleType.Real);
      try {
         workView.image.selectedChannel = 0;
         corrImg.selectedChannel = 0;
         corrImg.apply(workView.image, ImageOp.Mov);
...
function optSaveViewToFITS(view, filePath) {
   var src = view.image;
   var isFloat = (src.sampleType === PixelSampleType.Real);
   var isColor = (src.colorSpace !== ColorSpace.Gray);
...
function optRunCosmicClarityCLI(args, timeoutMs) {
   ...
   var maxMs = FMath.max(1000, timeoutMs || 300000);
   var candidates = [];
   ...
   for (var ci = 0; ci < candidates.length; ++ci) {
      ...
      while (proc.isStarting || proc.isRunning) {
         if ((new Date().getTime() - t0) > maxMs) {
            ...
            throw new Error("Cosmic Clarity timed out after " + FMath.round(maxMs / 1000) + " seconds: " + c.prog);
         }
         msleep(100);
         processEvents();
      }
...
function optRunBackgroundNeutralization(targetView) {
   ...
   var imgW = img.width;
   var imgH = img.height;
   var roiW = FMath.min(50, imgW);
   var roiH = FMath.min(50, imgH);
   var step = FMath.max(1, FMath.round(FMath.min(imgW, imgH) / 60));
...
function optMemoryAlgorithmInfo(tabName, stageName, actionKey, dlg, resultMeta) {
   ...
   if (actionKey === "post_sharp") {
      ...
      info.algorithm = shNames[FMath.max(0, FMath.min(shNames.length - 1, shIdx))];
...
function optEstimateViewBytes(view) {
   ...
   try {
      ...
      try { bps = FMath.max(1, FMath.floor(view.window.bitsPerSample / 8)); } catch (e0) { bps = 4; }
      return img.width * img.height * FMath.max(1, img.numberOfChannels) * bps;
   } catch (eE) {
      return 0;
   }
}
      ...
      if (optEvictSlotEntry(e)) {
         ...
         try {
            console.noteln("[Memory budget] Evicted " + e.label + " slot " + (e.index + 1) +
                           " (" + FMath.round(bytes / (1024 * 1024)) + " MB)" +
...
var OptPreviewControl = class extends ScrollBox {
   constructor(parent) {
   super(parent);
   this.bitmap = null;
   ...
   this.clampScrollPoint = function(p) {
      ...
      var maxY = 0;
      if (this.bitmap) {
         maxX = FMath.max(0, FMath.round(this.bitmap.width * this.scale) - this.viewport.width);
         maxY = FMath.max(0, FMath.round(this.bitmap.height * this.scale) - this.viewport.height);
      }
      return new Point(FMath.max(0, FMath.min(maxX, FMath.round(p.x))), FMath.max(0, FMath.min(maxY, FMath.round(p.y))));
   };

   this.updateScrollBars = function() {
      if (this.bitmap) {
         var imgW = FMath.round(this.bitmap.width * this.scale);
         var imgH = FMath.round(this.bitmap.height * this.scale);
         this.setHorizontalScrollRange(0, FMath.max(0, imgW - this.viewport.width));
         this.setVerticalScrollRange(0, FMath.max(0, imgH - this.viewport.height));
      } else {
   ...
   this.fitToWindow = function() {
      ...
      this.scale = FMath.max(0.05, FMath.min(FMath.min(sx, sy) * 0.98, 40.0));
      this.isFitMode = true;
   ...
   this.setManualScale = function(scale) {
      if (!this.bitmap)
         return;
      this.scale = FMath.max(0.05, FMath.min(scale, 40.0));
   ...
   this.setBitmap = function(bitmap, fit) {
      ...
      if (oldBitmap && oldScale > 0) {
         savedCenterX = ((saved.x / oldScale) + (this.viewport.width / (2 * oldScale))) / FMath.max(1, oldBitmap.width);
         savedCenterY = ((saved.y / oldScale) + (this.viewport.height / (2 * oldScale))) / FMath.max(1, oldBitmap.height);
         savedCenterX = FMath.max(0, FMath.min(1, savedCenterX));
         savedCenterY = FMath.max(0, FMath.min(1, savedCenterY));
      }
   ...
   this.paintBusyOverlay = function(g) {
      if (!this.busyActive)
         return;
      var x = 16, y = 16, r = 24;
      var w = FMath.max(220, FMath.min(360, 86 + (this.busyText ? this.busyText.length * 7 : 0)));
      var h = 62;
      ...
      g.drawTextRect(new Rect(x + 10, y + 7, x + 10 + 2 * r, y + 7 + 2 * r), "\u03C0", TextAlignment.Center | TextAlignment.VertCenter);
      if (this.busyText && this.busyText.length > 0)
         g.drawTextRect(new Rect(x + 70, y + 10, x + w - 12, y + h - 10), this.busyText, TextAlignment.Left | TextAlignment.VertCenter);
   };
   ...
   this.viewport.onPaint = function(x0, y0, x1, y1) {
      ...
      if (this.parent.bitmap) {
         try {
            ...
            var srcX = FMath.max(0, FMath.floor(sx / sc));
            var srcY = FMath.max(0, FMath.floor(sy / sc));
            var srcW = FMath.min(bmp.width - srcX, FMath.ceil(this.width / sc) + 2);
            var srcH = FMath.min(bmp.height - srcY, FMath.ceil(this.height / sc) + 2);
            if (srcW > 0 && srcH > 0) {
         ...
         g.drawTextRect(new Rect(0, 0, this.width, this.height), "Select Image", TextAlignment.Center);
         this.parent.paintBusyOverlay(g);
      }
      g.end();
   };
   ...
   this.viewport.onMousePress = function(x, y, button, buttons, modifiers) {
      ...
      var imgX = FMath.floor(((ctrl.scrollPosition.x + x) / ctrl.scale) * ctrl.imageCoordScaleX);
      var imgY = FMath.floor(((ctrl.scrollPosition.y + y) / ctrl.scale) * ctrl.imageCoordScaleY);
   ...
   this.viewport.onMouseWheel = function(x, y, delta) {
      ...
      newScale = FMath.max(0.05, FMath.min(newScale, 40.0));
...
function optEngineTitle(parent, text) {
   ...
   l.useRichText = true;
   l.text = "<b>" + text + "</b>";
   l.textAlignment = TextAlignment.Center | TextAlignment.VertCenter;
...
function optNumeric(parent, labelText, min, max, value, precision, labelWidth) {
   ...
   try {
      if (min >= 0 && max <= 1.0)
         nc.slider.setRange(FMath.round(min * 100), FMath.round(max * 100));
   } catch (e0) {}
...
function optSection(parent, title) {
   ...
   label.text = title;
   label.textAlignment = TextAlignment.Left | TextAlignment.VertCenter;
...
function OptPreviewPane(dialog, tab, parent) {
   ...
   this.preview.onZoomChanged = function(scale, isFit) {
      if (isFit)
         ...
         self.zoomCombo.editText = FMath.round(scale * 100) + "%";
   };
...
var PIWorkflowOptDialog = class extends Dialog {
   constructor() {
   super();
...
function OptStretchingEngine() {
   ...
   function computeMasRoiRobustStats(view, x0, y0, roiW, roiH) {
      ...
      var median = 0.0;
      if (n > 0) {
         var half = FMath.floor(n / 2);
         median = (n & 1) ? values[half] : 0.5 * (values[half - 1] + values[half]);
      }
      var deviations = [];
      deviations.length = n;
      for (var i = 0; i < n; ++i)
         deviations[i] = FMath.abs(values[i] - median);
      deviations.sort(function(a, b) { return a - b; });
      var mad = 0.0;
      if (n > 0) {
         var halfMad = FMath.floor(n / 2);
   ...
   function findMasBackgroundROI(view, requestedWidth, requestedHeight) {
      ...
      if (w <= 0 || h <= 0)
         return null;
      var roiW = FMath.max(1, FMath.min(FMath.round(requestedWidth || 25), w));
      var roiH = FMath.max(1, FMath.min(FMath.round(requestedHeight || 25), h));
      var maxX0 = FMath.max(0, w - roiW);
      var maxY0 = FMath.max(0, h - roiH);
      ...
      for (var iCand = 0; iCand < topCandidates.length; ++iCand) {
         ...
         for (var oy = -3; oy <= 3; ++oy) {
            for (var ox = -3; ox <= 3; ++ox) {
               var xx = FMath.max(0, FMath.min(maxX0, cand.x0 + ox));
               var yy = FMath.max(0, FMath.min(maxY0, cand.y0 + oy));
               var key = xx + "_" + yy;
   ...
   function configureMasProcessInstance(mas, targetView, params, isRGB) {
      ...
      var scaleSeparation = FMath.max(16, FMath.round(isFinite(params.ms_cr_scale) ? params.ms_cr_scale : 1024));
   ...
   function runStarStretch(view, params) {
      ...
      if (view.image.numberOfChannels >= 3) {
         ...
         if (removeGreen) {
            var S = new SCNR();
            S.amount = 1.00;
            S.protectionMethod = SCNR.AverageNeutral;
            S.colorToRemove = SCNR.Green;
   ...
   this.runStretch = function(view, algoId, params) {
      ...
      if (algoId === "STF") {
         ...
         eff_mid = FMath.min(0.999, FMath.max(0.001, eff_mid));
         for (var i = 0; i < c; ++i) {
            view.image.selectedChannel = i;
            var med = view.image.median();
            var mad = view.image.MAD();
            var sh = FMath.max(0, med + eff_shadow * mad);
            var val = med - sh;
            var m = 0.5;
            if (val > 0)
         ...
         if (params.stat_luma && isRGB) {
            var b = FMath.max(0, FMath.min(1, params.stat_blend));
         ...
         P.executeOn(view);
         if (params.stat_hdr) {
            var hdrLayers = FMath.max(3, FMath.min(8, FMath.round(6 * params.stat_hdramt + 2)));
            var hdrOverdrive = FMath.max(0, FMath.min(1, params.stat_hdrknee));
            try {
         ...
         if (params.stat_curve > 0) {
            ...
            C.Bt = CurvesTransformation.AkimaSubsplines;
            C.K = [
               [0.0, 0.0],
               [0.5 * params.stat_med, 0.5 * params.stat_med],
               [params.stat_med, params.stat_med],
               [(1 / 4 * (1 - params.stat_med) + params.stat_med), FMath.pow((1 / 4 * (1 - params.stat_med) + params.stat_med), (1 - params.stat_curve))],
               [(3 / 4 * (1 - params.stat_med) + params.stat_med), FMath.pow(FMath.pow((3 / 4 * (1 - params.stat_med) + params.stat_med), (1 - params.stat_curve)), (1 - params.stat_curve))],
               [1.0, 1.0]
            ];
...
function optEqualizeSkyBackgroundsBeforeStretch(view) {
   ...
   if (FMath.abs(mR - avgM) < 1.0e-9 && FMath.abs(mG - avgM) < 1.0e-9 && FMath.abs(mB - avgM) < 1.0e-9)
      return;
   var P = new PixelMath();
   P.clearImageCacheAndExit = false;
   P.cacheGeneratedImages = false;
...
function optBuildWorkflowTitleBar(parent) {
   ...
   logo.text = "\u03C0";
   logo.textAlignment = TextAlignment.Center | TextAlignment.VertCenter;
   logo.styleSheet =
...
PIWorkflowOptDialog.prototype.buildStretchCurvesWidget = function() {
   ...
   this.stretchCurvesWidget.cursor = new Cursor(StdCursor.Cross);
   ...
   this.stretchCurvesWidget.canvasToX = function(x) { var m = 10; return (x - m) / FMath.max(1, this.width - 2 * m); };
   this.stretchCurvesWidget.canvasToY = function(y) { var m = 10; return ((this.height - m) - y) / FMath.max(1, this.height - 2 * m); };
   ...
   this.stretchCurvesWidget.onPaint = function() {
      ...
      try {
         ...
         if (hist) {
            ...
            g.drawTextRect(new Rect(m, m, w - m, h - m), "Histogram", TextAlignment.Center | TextAlignment.VertCenter);
         }
         try { g.pen = new Pen(0xff404040, 1, PenStyle.Dash); } catch (eDash) { g.pen = new Pen(0xff404040, 1); }
   ...
   this.stretchCurvesWidget.onMouseMove = function(x, y) {
      ...
      if (this.__dragging >= 0 && this.__dragging < pts.length) {
         ...
         if (di === 0 || di === pts.length - 1)
            ...
            pts[di][0] = FMath.max(pts[di - 1][0] + 0.005, FMath.min(pts[di + 1][0] - 0.005, optClamp01(this.canvasToX(x))));
            pts[di][1] = ny;
         }
         zone.curvesPoints[key] = pts;
         this.__pts = pts;
...
function optExecuteNoiseXConfiguredOnView(targetView, cfg) {
   ...
   optTrySetProcessPropertySilently(nxt, ["iterations", "Iterations"], FMath.round(cfg.iterations));
...
function optExecuteTgvDenoiseOnView(targetView, dialog) {
   ...
   optTrySetProcessPropertySilently(tgv, ["maxIterations", "iterations"], FMath.round(optNumericValue(dialog.ncPostTgvIter, 500)));
...
function optExecuteHdrMtOnView(targetView, dialog) {
   ...
   optTrySetProcessPropertySilently(hdr, ["numberOfLayers", "layers"], FMath.round(optNumericValue(dialog.ncPostHdrLayers, 6)));
   optTrySetProcessPropertySilently(hdr, ["numberOfIterations", "iterations"], FMath.round(optNumericValue(dialog.ncPostHdrIter, 1)));
...
function optExecuteLheOnView(targetView, dialog) {
   ...
   optTrySetProcessPropertySilently(lhe, ["kernelRadius", "radius"], FMath.round(optNumericValue(dialog.ncPostLheRadius, 64)));
...
function optApplyColorBalanceFromState(view, state) {
   ...
   if (view.image.numberOfChannels >= 3 && (FMath.abs(r - 1) > 0.0001 || FMath.abs(g - 1) > 0.0001 || FMath.abs(b - 1) > 0.0001)) {
      var pm = new PixelMath();
      pm.useSingleExpression = false;
   ...
   if (view.image.numberOfChannels >= 3 && FMath.abs(optNumericValue(state ? state.saturation : null, 1.0) - 1.0) > 0.0001) {
      ...
      cs.HSt = ColorSaturation.AkimaSubsplines;
      cs.executeOn(view);
   }
   if (view.image.numberOfChannels >= 3 && optChecked(state ? state.scnr : null, false)) {
      ...
      scnr.protectionMethod = SCNR.AverageNeutral;
...
function optColorizeMono(view, hueDeg, saturation) {
   ...
   var sat = FMath.max(0.0, FMath.min(1.0, isFinite(saturation) ? saturation : 0.5));
...
function optApplyHueSaturationCorrectionToView(view, hueShiftTurns, saturationFactor) {
   if (!optSafeView(view) || view.image.numberOfChannels < 3)
      return view;
   var shift = FMath.max(-0.25, FMath.min(0.25, isFinite(hueShiftTurns) ? hueShiftTurns : 0.0));
   var sat = FMath.max(0.0, FMath.min(4.0, isFinite(saturationFactor) ? saturationFactor : 1.0));
   if (FMath.abs(shift) <= 1.0e-6 && FMath.abs(sat - 1.0) <= 0.001)
      return view;
   ...
   try {
      ...
      var cc = new ChannelCombination();
      cc.colorSpace = ChannelCombination.HSI;
      cc.channels = [[true, hView.id], [true, sView.id], [true, iView.id]];
      cc.executeOn(view);
   } finally {
      optCloseView(hView);
...
function optPostCurvePoints(dialog) {
   ...
   function c01(v) { return FMath.max(0, FMath.min(1, v)); }
   return [
      [0.00, c01(shadows)],
...
function optApplyCurvesFromState(view, channelIndex, pointsMap, controls) {
   ...
   if (view.image.numberOfChannels >= 3 && FMath.abs(sat - 1.0) > 0.0001)
      ct.S = [[0, 0], [0.5, FMath.max(0, FMath.min(1, 0.5 * sat))], [1, FMath.max(0, FMath.min(1, sat))]];
   ct.executeOn(view);
   return view;
}
...
function optAkimaInterpolate(points, numOut) {
   if (!points || points.length < 2) {
      ...
      for (var ii = 0; ii < numOut; ++ii)
         id.push(ii / FMath.max(1, numOut - 1));
      return id;
   }
   ...
   if (pts.length === 2) {
      ...
      for (var i = 0; i < numOut; ++i) {
         var x = i / FMath.max(1, numOut - 1);
         var t = (x - pts[0][0]) / FMath.max(1.0e-12, pts[1][0] - pts[0][0]);
         t = optClamp01(t);
   ...
   for (var j = 0; j < n - 1; ++j) {
      dx[j] = FMath.max(1.0e-12, pts[j + 1][0] - pts[j][0]);
   ...
   for (var k = 0; k < n; ++k) {
      if (k === 0) tang[k] = m[0];
         ...
         var w1 = FMath.abs(mm3 - mm2) + 1.0e-12;
         var w2 = FMath.abs(mm1 - mm0) + 1.0e-12;
         tang[k] = (w1 * mm1 + w2 * mm2) / (w1 + w2);
      }
   }
   var out = [];
   for (var o = 0; o < numOut; ++o) {
      var xx = o / FMath.max(1, numOut - 1);
      var seg = 0;
      for (var s = 0; s < n - 1; ++s)
...
function optPostHueSatFromRgb(r, g, b) {
   var mx = FMath.max(r, g, b);
   var mn = FMath.min(r, g, b);
   var d = mx - mn;
...
function optPostRangeWeight(v, low, high, fuzz, binary) {
   var lo = FMath.min(low, high), hi = FMath.max(low, high);
   if (binary)
      return (v >= lo && v <= hi) ? 1 : 0;
   if (v >= lo && v <= hi) return 1;
   if (fuzz > 0 && v >= lo - fuzz && v < lo) return FMath.max(0, (v - (lo - fuzz)) / fuzz);
   if (fuzz > 0 && v > hi && v <= hi + fuzz) return FMath.max(0, ((hi + fuzz) - v) / fuzz);
   return 0;
}
...
function optPostFameAngle(cx, cy, x, y) { return FMath.atan2(y - cy, x - cx); }
...
function optPostFameBuildEllipsePoints(x0, y0, x1, y1) {
   ...
   var rx = FMath.abs(x1 - x0) / 2, ry = FMath.abs(y1 - y0) / 2;
   var steps = FMath.max(32, FMath.round(2 * FMath.PI * FMath.max(rx, ry)));
   var pts = [];
   for (var i = 0; i <= steps; ++i)
      pts.push([cx + rx * FMath.cos(2 * FMath.PI * i / steps), cy + ry * FMath.sin(2 * FMath.PI * i / steps)]);
   return pts;
}
...
function optPostFameTransformShape(shape, angle, scale, cx, cy) {
   ...
   for (var i = 0; i < arr.length; ++i) {
      ...
      var dist = FMath.sqrt(dx * dx + dy * dy) * scale;
      var ang = FMath.atan2(dy, dx) + angle;
      arr[i][0] = cx + dist * FMath.cos(ang);
      arr[i][1] = cy + dist * FMath.sin(ang);
   }
}
...
function optPostFamePixelValue(srcImg, x, y, mode, colorRange, gradState) {
   ...
   if (mode === "Chrominance") {
      if (nch < 3) return 0;
      var mx = FMath.max(r, g, b), mn = FMath.min(r, g, b);
   ...
   if (mode === "Gradient" && gradState && gradState.A && gradState.B) {
      ...
      var len = FMath.sqrt(gdx * gdx + gdy * gdy);
      if (len <= 1.0e-10) return 0;
      var vx = x - gradState.A[0], vy = y - gradState.A[1];
      return FMath.max(0, FMath.min(1, (vx * gdx + vy * gdy) / (len * len)));
   }
   return 0;
}
...
function optPostFameFillPolygon(outImg, srcImg, polygon, pixelFn, replaceOnly) {
   ...
   for (var y = FMath.max(0, FMath.floor(minY)); y <= FMath.min(h - 1, FMath.ceil(maxY)); ++y) {
      var xs = [];
      for (var p = 0; p < polygon.length; ++p) {
         var q = (p + 1) % polygon.length;
         var y1 = polygon[p][1], y2 = polygon[q][1];
         if ((y1 <= y && y < y2) || (y2 <= y && y < y1))
            xs.push(FMath.round(polygon[p][0] + (y - y1) * (polygon[q][0] - polygon[p][0]) / (y2 - y1)));
      }
      xs.sort(function(a, b) { return a - b; });
      for (var k = 0; k + 1 < xs.length; k += 2) {
         for (var x = FMath.max(0, xs[k]); x <= FMath.min(w - 1, xs[k + 1]); ++x) {
            var v = FMath.max(0, FMath.min(1, pixelFn(srcImg, x, y)));
...
function optPostFameRasterizeCircle(outImg, srcImg, cx, cy, radius, pixelFn, replaceOnly) {
   radius = FMath.max(1, radius);
   var w = outImg.width, h = outImg.height, r2 = radius * radius;
   for (var y = FMath.max(0, FMath.floor(cy - radius)); y <= FMath.min(h - 1, FMath.ceil(cy + radius)); ++y)
      for (var x = FMath.max(0, FMath.floor(cx - radius)); x <= FMath.min(w - 1, FMath.ceil(cx + radius)); ++x)
...
function optPostFameFillBrush(outImg, srcImg, shape, pixelFn, replaceOnly) {
   var centers = shape.centers || [], radius = FMath.max(1, shape.radius || 10);
   ...
   for (var s = 0; s < centers.length - 1; ++s) {
      ...
      var steps = FMath.max(1, FMath.ceil(optPostFameDistance(p0[0], p0[1], p1[0], p1[1]) / FMath.max(1, radius * 0.35)));
...
function optPostFameFillSpray(outImg, srcImg, shape, pixelFn, replaceOnly) {
   ...
   for (var i = 0; i < pts.length; ++i) {
      var x = FMath.round(pts[i][0]), y = FMath.round(pts[i][1]);
      if (x >= 0 && y >= 0 && x < w && y < h) {
         var v = FMath.max(0, FMath.min(1, pixelFn(srcImg, x, y)));
         if (v > 0 && (replaceOnly || v > outImg.sample(x, y, 0))) outImg.setSample(v, x, y, 0);
      }
...
function optPostFameAppendSprayPoints(shape, cx, cy, radius, density) {
   var count = FMath.max(1, FMath.round(FMath.PI * radius * radius * FMath.max(0.01, density) * 0.15));
   for (var i = 0; i < count; ++i) {
      var ang = FMath.random() * 2 * FMath.PI;
      var dist = FMath.random() * radius;
      shape.points.push([cx + dist * FMath.cos(ang), cy + dist * FMath.sin(ang)]);
   }
}
...
function optBuildPostFameMaskImage(tv, dialog) {
   ...
   var srcImg = tv.image;
   var outImg = new Image(srcImg.width, srcImg.height, 1, ColorSpace.Gray, 32, PixelSampleType.Real);
   outImg.fill(0);
...
function optRenderFameOverlay(g, sc, sx, sy, fameState, coordScaleX, coordScaleY) {
   ...
   function toScreen(ix, iy) { return { x: FMath.round((ix / kx) * sc - sx), y: FMath.round((iy / ky) * sc - sy) }; }
   function drawShape(shape, active) {
      ...
      if (shape.type === "Brush" || shape.type === "SprayCan") {
         var rad = FMath.max(1, FMath.round((shape.radius || 10) * sc / FMath.max(kx, ky)));
...
OptPostMaskLiveCache.prototype.release = function() {
   ...
   if (this.bitmap)
      try { this.bitmap.clear(); } catch (e1) {}
   this.key = "";
   this.workImg = null;
   this.freeWork = false;
   this.W = 0;
...
function optPostMaskLiveBitmap(cache, width, height) {
   ...
   if (!cache.bitmap || cache.bitmapKey !== key) {
      if (cache.bitmap)
         try { cache.bitmap.clear(); } catch (e0) {}
...
function optPostMaskPreviewBitmapSize(dialog, sourceView) {
   ...
   try {
      return {
         width: FMath.max(1, FMath.round(sourceView.image.width / reduction)),
         height: FMath.max(1, FMath.round(sourceView.image.height / reduction))
      };
   } catch (e1) {
   }
...
function optPreparePostMaskWorkImage(sourceView, live, cache) {
   ...
   if (live && FMath.max(srcW, srcH) > OPT_POST_LIVE_MAX_DIM) {
      var f = OPT_POST_LIVE_MAX_DIM / FMath.max(srcW, srcH);
      W = FMath.max(1, FMath.round(srcW * f));
      H = FMath.max(1, FMath.round(srcH * f));
   }
   if (live && cache) {
      ...
      cache.release();
      if (W !== srcW || H !== srcH) {
         workImg = new Image(srcW, srcH, srcImg.numberOfChannels, srcImg.colorSpace, 32, PixelSampleType.Real);
         workImg.assign(srcImg);
         workImg.resample(W, H, InterpolationAlgorithm.Bilinear);
         freeWork = true;
      }
      cache.key = key;
...
function optRenderPostMaskBitmap(maskArr, work, binary, targetW, targetH) {
   var outW = FMath.max(1, FMath.round(targetW || work.W));
   var outH = FMath.max(1, FMath.round(targetH || work.H));
   var bmp = optPostMaskLiveBitmap(work.cache || null, outW, outH);
   var scaleX = work.W / outW;
   var scaleY = work.H / outH;
   for (var y = 0; y < outH; ++y) {
      var sy = FMath.min(work.H - 1, FMath.floor(y * scaleY));
      var row = sy * work.W;
      for (var x = 0; x < outW; ++x) {
         var sx = FMath.min(work.W - 1, FMath.floor(x * scaleX));
         var g = FMath.max(0, FMath.min(255, FMath.round(maskArr[row + sx] * 255)));
...
function optRenderMaskViewPreviewBitmap(maskView, dialog) {
   ...
   var outW = FMath.max(1, size.width);
   var outH = FMath.max(1, size.height);
   ...
   for (var y = 0; y < outH; ++y) {
      var sy = FMath.min(srcH - 1, FMath.floor(y * scaleY));
      img.getSamples(row, new Rect(0, sy, srcW, sy + 1), 0);
      for (var x = 0; x < outW; ++x) {
         var sx = FMath.min(srcW - 1, FMath.floor(x * scaleX));
         var g = FMath.max(0, FMath.min(255, FMath.round(row[sx] * 255)));
         bmp.setPixel(x, y, 0xff000000 | (g << 16) | (g << 8) | g);
      }
   }
...
function optFinishPostMaskView(maskArr, work, sourceView, live, baseId, smooth) {
   var mask = new Image(work.W, work.H, 1, ColorSpace.Gray, 32, PixelSampleType.Real);
   try {
      mask.setSamples(maskArr, new Rect(0, 0, work.W, work.H), 0);
      if (live && (work.W !== work.srcW || work.H !== work.srcH))
         mask.resample(work.srcW, work.srcH, InterpolationAlgorithm.Bilinear);
   ...
   try {
      ...
      try {
         for (var y = 0; y < height; y += tileRows) {
            var h = FMath.min(tileRows, height - y);
            var n = width * h;
...
function optFillRangeMaskRgbBrightness(maskArr, rArr, gArr, bArr, n, low, high, fuzz, invert, binary) {
   ...
   if (binary) {
      if (invert) {
         for (var i0 = 0; i0 < n; ++i0) {
            var v0 = FMath.max(rArr[i0], gArr[i0], bArr[i0]);
            maskArr[i0] = (v0 >= lo && v0 <= hi) ? 0 : 1;
         }
      } else {
         for (var i1 = 0; i1 < n; ++i1) {
            var v1 = FMath.max(rArr[i1], gArr[i1], bArr[i1]);
            maskArr[i1] = (v1 >= lo && v1 <= hi) ? 1 : 0;
         }
      }
      return;
   ...
   if (invert) {
      for (var i2 = 0; i2 < n; ++i2) {
         var v2 = FMath.max(rArr[i2], gArr[i2], bArr[i2]);
         maskArr[i2] = 1 - optPostRangeWeight(v2, lo, hi, fuzz, false);
      }
   } else {
      for (var i3 = 0; i3 < n; ++i3) {
         var v3 = FMath.max(rArr[i3], gArr[i3], bArr[i3]);
         maskArr[i3] = optPostRangeWeight(v3, lo, hi, fuzz, false);
      }
   }
}
...
function optApplySmoothToMaskArr(maskArr, W, H, sigma) {
   ...
   var radius = FMath.floor(kernel.length / 2);
      ...
      for (var x = 0; x < W; ++x) {
         ...
         if (x >= radius && x < W - radius) {
            ...
            for (var ke = 0; ke < kLen; ++ke) {
               var xi = FMath.max(0, FMath.min(W - 1, x + ke - radius));
      ...
      for (var y2 = 0; y2 < H; ++y2) {
         ...
         if (y2 >= radius && y2 < H - radius) {
            ...
            for (var k2e = 0; k2e < kLen; ++k2e) {
               var yi = FMath.max(0, FMath.min(H - 1, y2 + k2e - radius));
...
function optBuildPostRangeMaskView(sourceView, dialog, opts) {
   ...
   var fuzz = FMath.max(0, optNumericValue(dialog.ncPostRangeFuzz, 0.05));
   ...
   try {
      ...
      if (smooth > 0.0) {
         var liveSigma = smooth * work.W / FMath.max(1, work.srcW);
         if (liveSigma > 0.1)
...
function optBuildPostColorMaskView(sourceView, dialog, opts) {
   ...
   try {
      ...
      if (cmSmooth > 0.0) {
         var cmLiveSigma = cmSmooth * work.W / FMath.max(1, work.srcW);
         if (cmLiveSigma > 0.1)
...
function optGaussianKernelForSigma(sigma) {
   var s = FMath.max(0.001, sigma);
   var radius = FMath.max(1, FMath.ceil(3 * s));
...
function optLiveCandidateMaxDim(dialog, referenceView) {
   ...
   try {
      if (optSafeView(referenceView))
         longest = FMath.max(referenceView.image.width, referenceView.image.height);
   } catch (e0) {}
   ...
   try {
      ...
      if (isFinite(reduction) && reduction > 1)
         maxDim = FMath.min(maxDim, FMath.max(128, FMath.round(longest / reduction)));
   } catch (e1) {}
   return FMath.max(128, FMath.min(longest, maxDim));
}
...
function optApplyCcSlotCurvesToView(view, points) {
   ...
   var identity = points.length === 2 &&
      FMath.abs(points[0][0]) < 0.001 && FMath.abs(points[0][1]) < 0.001 &&
      FMath.abs(points[1][0] - 1.0) < 0.001 && FMath.abs(points[1][1] - 1.0) < 0.001;
   if (identity)
      return view;
   var ct = new CurvesTransformation();
   ct.K = points;
...
function optUpdateCcSlotColorStats(dialog, slot, force) {
   ...
   if (force === true || !slot.__colorStatsReady) {
      if (src.image.numberOfChannels >= 3) {
         ...
         slot.colorPointIntensity = FMath.max(0.65, stats.sat);
      } else {
...
function optPrepareCcSlotView(dialog, slot, opts) {
   ...
   try {
      ...
      if (optSafeView(slotMask)) {
         ...
         if (live && (work.image.width !== slotMask.image.width ||
            ...
            if (optSafeView(tempMask))
               optDownsamplePreparedView(tempMask, FMath.max(work.image.width, work.image.height));
      ...
      if (FMath.abs(bright - 1.0) > 0.0001)
      ...
      if (work.image.numberOfChannels >= 3 && FMath.abs(sat - 1.0) > 0.0001) {
         var cs = new ColorSaturation();
...
function optDownsamplePreparedView(view, maxDim) {
   ...
   var maxD = FMath.max(W, H);
   if (maxD <= maxDim)
      return false;
   var scale = maxDim / maxD;
   var newW = FMath.max(1, FMath.round(W * scale));
   var newH = FMath.max(1, FMath.round(H * scale));
   try {
      view.beginProcess(UndoFlag.NoSwapFile);
      view.image.resample(newW, newH, InterpolationAlgorithm.Bilinear);
      view.endProcess();
      return true;
   } catch (eR) {
...
function optCcLivePreviewMaxDim(dialog, referenceView) {
   ...
   var maxDim = FMath.max(128, FMath.round(longest / FMath.max(1, reduction)));
   return FMath.max(128, FMath.min(longest, maxDim));
}
...
function optReleaseCcSlotCaches(dialog) {
   ...
   for (var i = 0; i < dialog.ccSlots.length; ++i) {
      var slot = dialog.ccSlots[i];
      optInvalidateCcSlotCache(slot, "all");
      if (slot && slot.colourWheelBmp) {
         try { slot.colourWheelBmp.clear(); } catch (eBmp) {}
         slot.colourWheelBmp = null;
      }
   }
}
      ...
      build: function(body) {
         ...
         dlg.updatePostColorBalanceStats = function(force) {
            ...
            if (force === true || !dlg.__postBalanceStatsReady) {
               ...
               dlg.postBalancePointIntensity = FMath.max(0.65, stats.sat);
         ...
         dlg.pickPostColorBalanceWheel = function(x, y) {
            ...
            var sz = FMath.min(w, h);
            var cx = w * 0.5;
            var cy = h * 0.5;
            var outer = sz * 0.5 - 2.0;
            var dx = x - cx;
            var dy = y - cy;
            var dist = FMath.sqrt(dx * dx + dy * dy);
            if (dist > outer) {
               var k = outer / FMath.max(1.0e-6, dist);
               dx *= k; dy *= k; dist = outer;
            }
            dlg.postBalancePointIntensity = optClamp01(dist / FMath.max(1.0e-6, outer));
            var a = FMath.atan2(dy, dx) * 180.0 / FMath.PI;
         ...
         dlg.postColorBalanceWheel.cursor = new Cursor(StdCursor.Cross);
         dlg.postColorBalanceWheel.onPaint = function() {
            var g = new Graphics(this);
            try {
               var sz = FMath.min(this.width, this.height);
               var cx = this.width * 0.5;
               var cy = this.height * 0.5;
               if (!dlg.postBalanceWheelBmp || dlg.postBalanceWheelBmp.width !== sz) {
                  if (dlg.postBalanceWheelBmp)
                     try { dlg.postBalanceWheelBmp.clear(); } catch (eOldBmp) {}
                  dlg.postBalanceWheelBmp = optGenerateHueWheelBitmap(sz, 0.0);
               }
               g.drawBitmap(0, 0, dlg.postBalanceWheelBmp);
               var outer = sz * 0.5 - 2.0;
               var meanRad = dlg.postBalanceMeanHueDeg * FMath.PI / 180.0;
               g.pen = new Pen(0xffffffff, 2);
               g.drawLine(cx, cy, cx + FMath.cos(meanRad) * outer * 0.65, cy + FMath.sin(meanRad) * outer * 0.65);
               var ptRad = dlg.postBalancePointHueDeg * FMath.PI / 180.0;
               var pr = outer * optClamp01(dlg.postBalancePointIntensity);
               var px = cx + FMath.cos(ptRad) * pr;
               var py = cy + FMath.sin(ptRad) * pr;
...
function optBuildPostCurvesSection(dlg) {
   dlg.postTab.addProcessSection("Curves", [{
      ...
      build: function(body) {
         ...
         dlg.postCurvesWidget.cursor = new Cursor(StdCursor.Cross);
         ...
         dlg.postCurvesWidget.canvasToX = function(x) { var m = 10; return (x - m) / FMath.max(1, this.width - 2 * m); };
         dlg.postCurvesWidget.canvasToY = function(y) { var m = 10; return ((this.height - m) - y) / FMath.max(1, this.height - 2 * m); };
...
function optBuildPostMaskingSection(dlg) {
   dlg.postTab.addProcessSection("Masking", [], {
      build: function(body) {
         ...
         dlg.postRangeStrip.cursor = new Cursor(StdCursor.Cross);
         ...
         dlg.postRangeStrip.onPaint = function() {
            ...
            try {
               ...
               for (var x = 0; x < w; ++x) {
                  var v = FMath.round(255 * x / FMath.max(1, w - 1));
               ...
               var lx = FMath.round(low * (w - 1)), hx = FMath.round(high * (w - 1));
               g.pen = new Pen(0xFFFFFFFF, 1); g.drawRect(new Rect(FMath.min(lx,hx), 1, FMath.max(lx,hx)+1, h-1));
         ...
         dlg.postRangeStrip.onMousePress = function(x, y, button) {
            if (button !== OPT_MOUSE_LEFT) return;
            var lx = FMath.round(dlg.ncPostRangeLow.value * (this.width - 1));
            var hx = FMath.round(dlg.ncPostRangeHigh.value * (this.width - 1));
            dlg.postRangeStripDragging = (FMath.abs(x - lx) <= FMath.abs(x - hx)) ? "low" : "high";
            var v = FMath.max(0, FMath.min(1, x / FMath.max(1, this.width - 1)));
            if (dlg.postRangeStripDragging === "low") dlg.ncPostRangeLow.setValue(FMath.min(v, dlg.ncPostRangeHigh.value));
            else dlg.ncPostRangeHigh.setValue(FMath.max(v, dlg.ncPostRangeLow.value));
            this.repaint();
         };
         ...
         dlg.postHueWheel.cursor = new Cursor(StdCursor.Cross);
         ...
         dlg.postHueWheel.onPaint = function() {
            ...
            try {
               ...
               if (!dlg._postHueWheelBmp || dlg._postHueWheelBmp.width !== sz) {
                  if (dlg._postHueWheelBmp)
                     try { dlg._postHueWheelBmp.clear(); } catch (eOldHue) {}
               ...
               var hueRad = dlg.ncPostCMHue.value / 360.0 * 2 * FMath.PI - FMath.PI / 2;
               var hueRange = dlg.ncPostCMHueRange.value / 360.0 * 2 * FMath.PI;
               g.pen = new Pen(0xFFFFFFFF, 2);
               g.drawLine(cx, cy, FMath.round(cx + outerR * FMath.cos(hueRad)), FMath.round(cy + outerR * FMath.sin(hueRad)));
               g.pen = new Pen(0xFFFFFFFF, 1);
               var r1 = hueRad - hueRange / 2, r2 = hueRad + hueRange / 2;
               g.drawLine(cx, cy, FMath.round(cx + outerR * FMath.cos(r1)), FMath.round(cy + outerR * FMath.sin(r1)));
               g.drawLine(cx, cy, FMath.round(cx + outerR * FMath.cos(r2)), FMath.round(cy + outerR * FMath.sin(r2)));
            } finally { g.end(); }
         };
         dlg.postHueWheel.onMousePress = function(x, y, button) {
            if (button !== OPT_MOUSE_LEFT) return;
            var cx = this.width / 2, cy = this.height / 2;
            var ang = FMath.atan2(x - cx, -(y - cy));
            if (ang < 0) ang += 2 * FMath.PI;
            var hueDeg = ang * 180 / FMath.PI;
         ...
         dlg.postHueWheel.onMouseMove = function(x, y) {
            ...
            if (dlg.postHueWheelDragMode === "center") {
               ...
               var d = FMath.abs(hueDeg - center);
               if (d > 180) d = 360 - d;
               dlg.ncPostCMHueRange.setValue(FMath.max(1, FMath.min(180, d)));
            }
            this.repaint();
         };
         ...
         dlg.installPostFameHooks = function() {
            ...
            previewCtrl.onImageMousePress = function(imgX, imgY, button, modifiers) {
               ...
               if ((modifiers & ALT) && st.shapes.length > 0) {
                  ...
                  st.initialDistance = FMath.max(1.0e-6, optPostFameDistance(imgX, imgY, st.transformCenter[0], st.transformCenter[1]));
                  return true;
               }
            ...
            previewCtrl.onImageMouseMove = function(imgX, imgY, buttons, modifiers) {
               ...
               if (st.isDrawing && st.currentShape) {
                  ...
                  if (stype === "Freehand")        st.currentShape.points.push([imgX, imgY]);
                     ...
                     if (!last || optPostFameDistance(last[0],last[1],imgX,imgY) >= FMath.max(1, st.currentShape.radius * 0.35))
                  ...
                  var curDist  = FMath.max(1.0e-6, optPostFameDistance(imgX, imgY, st.transformCenter[0], st.transformCenter[1]));
                  st.shapes[st.activeShapeIndex] = optPostFameCloneShape(st.originalShape);
...
PIWorkflowOptDialog.prototype.configureCcTab = function() {
   ...
   function buildCcSlotSection(slotIndex) {
      dlg.ccTab.addProcessSection("Image " + slotIndex, [], {
         build: function(body) {
            ...
            slot.colourWheel.cursor = new Cursor(StdCursor.Cross);
            ...
            slot.colourWheel.onPaint = function() {
               ...
               try {
                  ...
                  if (!s.colourWheelBmp || s.colourWheelBmp.width !== sz) {
                     if (s.colourWheelBmp)
                        try { s.colourWheelBmp.clear(); } catch (eOldColourWheel) {}
                     s.colourWheelBmp = optGenerateHueWheelBitmap(sz, 0.0);
                  }
                  g.drawBitmap(0, 0, s.colourWheelBmp);
                  var meanRad = (s.colorMeanHueDeg || 0.0) * FMath.PI / 180.0;
                  g.pen = new Pen(0xffffffff, 2);
                  g.drawLine(cx, cy, cx + FMath.cos(meanRad) * outerR * 0.65, cy + FMath.sin(meanRad) * outerR * 0.65);
                  var pointRad = (s.colorPointHueDeg || 0.0) * FMath.PI / 180.0;
                  var pointR = outerR * optClamp01(s.colorPointIntensity || 0.0);
                  var px = cx + pointR * FMath.cos(pointRad);
                  var py = cy + pointR * FMath.sin(pointRad);
                  g.pen = new Pen(0xffffff00, 2);
                  g.brush = new Brush(0xffffff00);
            ...
            slot.colourWheel.pick = function(x, y) {
               ...
               var dist = FMath.sqrt(dx * dx + dy * dy);
               if (dist > outerR) {
                  var scale = outerR / FMath.max(1.0e-6, dist);
                  dx *= scale; dy *= scale; dist = outerR;
               }
               var ang = FMath.atan2(dx, -dy);
               if (ang < 0.0)
                  ang += 2.0 * FMath.PI;
               s.colorPointHueDeg = (ang * 180.0 / FMath.PI) % 360.0;
               s.colorPointIntensity = optClamp01(dist / FMath.max(1.0e-6, outerR));
   ...
   dlg.ccCurvesWidget = new Control(this.ccTab.preview.control);
         ...
         dlg.ccCurvesWidget.cursor = new Cursor(StdCursor.Cross);
         ...
         dlg.ccCurvesWidget.canvasToX = function(x) { var m = 8; return (x - m) / FMath.max(1, this.width - 2 * m); };
         dlg.ccCurvesWidget.canvasToY = function(y) { var m = 8; return ((this.height - m) - y) / FMath.max(1, this.height - 2 * m); };
         ...
         dlg.ccCurvesWidget.onPaint = function() {
            ...
            try {
               ...
               if (hist) {
                  ...
                  g.pen = new Pen(0xff707070, 1);
                  g.drawTextRect(new Rect(m, m, w - m, h - m), "Select an Image slot to see its histogram", TextAlignment.Center | TextAlignment.VertCenter);
               }
               try { g.pen = new Pen(0xff404040, 1, PenStyle.Dash); } catch (eDash) { g.pen = new Pen(0xff404040, 1); }
               g.drawLine(m, h - m, w - m, m);
               var pts = this.__pts || [[0, 0], [1, 1]];
   ...
   try {
      if (!OPT_TEST_MODE && typeof StarXTerminator !== "undefined") {
         ...
         try {
            ...
            starlessWindow.mainView.beginProcess(UndoFlag.NoSwapFile);
...
PIWorkflowOptDialog.prototype.finalCleanup = function() {
   ...
   this._postLiveMask = null;
   this._postLiveMaskBitmap = null;
   if (this.postBalanceWheelBmp) {
      try { this.postBalanceWheelBmp.clear(); } catch (ePB) {}
      this.postBalanceWheelBmp = null;
   }
   if (this._postHueWheelBmp) {
      try { this._postHueWheelBmp.clear(); } catch (eHB) {}
      this._postHueWheelBmp = null;
   }
