...
> La sección *Novedades* de `cabraspace.com/cabrascripts.html#pi-workflow` lee en vivo el CHANGELOG de este repo. En cada release, añade el bloque de la nueva versión ARRIBA DEL TODO **solo en `CHANGELOG.md`** (inglés), con formato `## [X.X] - AAAA-MM-DD`, títulos `### ...`, viñetas `- **negrita** ...`. NO edites `CHANGELOG.es.md` a mano para la última versión: el workflow `translate-changelog` (`.github/workflows/`) lo traduce al español con GitHub Models y lo committea solo al hacer push a `main`. Detalle completo en `PUBLISH_GUIDE.md`.
