...
function optRecipeChannels(recipe) {
   ...
   // GOLDEN is a weighted TINT blend, not a pure channel mapping (see
   // optCreateGoldenNbFromChannels). These are the DOMINANT lines per output
   // channel, used only for narrowband metadata (PIWNBR/G/B, SPCC hints).
   if (r === "GOLDEN") return ["H", "S", "O"];
   return ["S", "H", "O"];
}

// =========================================================================
// GOLDEN-PALETTE-BEGIN — Added 2026-07-08 (build 31, user request)
// "Dorada / Golden": weighted tint palette instead of a 1:1 channel mapping.
//   Ha  -> old gold    (R 1.00, G 0.65, B 0.15)
//   OIII-> violet      (R 0.40, G 0.10, B 1.00)
//   SII -> blue-cyan   (R 0.00, G 0.80, B 1.00)
// Each output channel is the weighted sum of the mono lines, truncated to
// [0,1] (no rescale: per-channel rescale would destroy the tint balance).
// SII is OPTIONAL: with H+O only, the S terms are dropped (dual-band data
// always has the three DBXtract outputs, so this only matters for monos).
// Best applied to STRETCHED monos; on linear data the blend stays linear and
// the later stretch of the combined RGB shifts the tint balance.
// To revert: delete this block + the GOLDEN branches in ui/tabs_core.js
// combineNb() + the GOLDEN entries in optRecipeChannels /
// optIsNarrowbandRecipeName / OPT_RECIPE_NAMES / tooltips / help.
// =========================================================================
function optCreateGoldenNbFromChannels(viewH, viewO, viewS, baseId, metadataView) {
   if (!optSafeView(viewH) || !optSafeView(viewO))
      throw new Error("Golden palette: H and O channels are required (S is optional).");
   var hasS = optSafeView(viewS);
   var views = hasS ? [viewH, viewO, viewS] : [viewH, viewO];
   optRequireSameGeometry("Golden palette combination", views);
   var h = viewH.id, o = viewO.id, s = hasS ? viewS.id : null;
   // Build 33 — auto background neutralization so the palette looks right ON
   // FIRST LOAD, also on LINEAR monos (user report: raw combine came out solid
   // blue because B's weight sum (2.15) dominates when the three sky pedestals
   // are similar). Three-part fix, all inside the PixelMath expressions:
   //   1) subtract each mono's MEDIAN (its sky pedestal) so per-filter sky
   //      levels cannot tint the background;
   //   2) NORMALIZE each output channel by its weight sum (R/1.40, G/1.55,
   //      B/2.15) so equal signal excess maps to neutral, and only the LINE
   //      RATIOS drive the tint;
   //   3) add back a small NEUTRAL pedestal p = min(medians) so the background
   //      keeps its natural level (identical in R/G/B -> colorless).
   var mH = 0, mO = 0, mS = 0;
   try { mH = viewH.image.median(); } catch (eMh) {}
   try { mO = viewO.image.median(); } catch (eMo) {}
   if (hasS) { try { mS = viewS.image.median(); } catch (eMs) {} }
   var p = Math.min(mH, mO); if (hasS) p = Math.min(p, mS);
   function n(x) { return x.toFixed(8); }
   var tH = "(" + h + " - " + n(mH) + ")";
   var tO = "(" + o + " - " + n(mO) + ")";
   var tS = hasS ? "(" + s + " - " + n(mS) + ")" : null;
   var P = n(p) + " + ";
   var exprR, exprG, exprB;
   if (hasS) {
      exprR = P + "(" + tH + " + 0.40*" + tO + ")/1.40";
      exprG = P + "(0.65*" + tH + " + 0.10*" + tO + " + 0.80*" + tS + ")/1.55";
      exprB = P + "(0.15*" + tH + " + " + tO + " + " + tS + ")/2.15";
   } else {
      exprR = P + "(" + tH + " + 0.40*" + tO + ")/1.40";
      exprG = P + "(0.65*" + tH + " + 0.10*" + tO + ")/0.75";
      exprB = P + "(0.15*" + tH + " + " + tO + ")/1.15";
   }
   var win = null;
   var inProcess = false;
   try {
      win = optCreateWindowLike(viewH, baseId || "Opt_Golden", 3, true);
      win.mainView.beginProcess(UndoFlag_NoSwapFile);
      inProcess = true;
      var pm = new PixelMath();
      pm.useSingleExpression = false;
      ...
      win.mainView.endProcess();
      inProcess = false;
      optCopyMetadata(win, metadataView || viewH);
      try { win.hide(); } catch (eHide) {}
      console.writeln("=> Golden palette: Ha->old gold, OIII->violet" + (hasS ? ", SII->blue-cyan" : " (no SII)") +
                      " · background neutralized (medians " + mH.toFixed(5) + "/" + mO.toFixed(5) + (hasS ? "/" + mS.toFixed(5) : "") +
                      " subtracted, normalized weights, neutral pedestal " + p.toFixed(5) + ").");
      return win.mainView;
   } catch (e) {
      if (inProcess && win && !win.isNull) {
         try { win.mainView.endProcess(); } catch (eEnd) {}
...
function optIsNarrowbandRecipeName(text) {
   ...
   if (r === "SHO" || r === "HOO" || r === "HSO" || r === "HOS" || r === "OSS" ||
       ...
       r === "REAL1" || r === "REAL2" || r === "FORAXX" || r === "GOLDEN")
      return true;
   return false;
}
