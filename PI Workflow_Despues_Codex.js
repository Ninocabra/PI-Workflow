...
#feature-id    Utilities > PI Workflow
#feature-info  Optimized PI Workflow prototype v6d — native GraXpert process, narrowband-aware calibration, contextual help, repository manifest, and CC preview preservation.
...
// When a memory store would push usage above this cap, the oldest non-selected
// slots are released until usage falls back under the cap. Default ~1.5 GB
// keeps a comfortable margin on 16 GB systems running PixInsight alongside
// the OS and other tools.
...
function optEnsureVeraLuxSupportLoaded() {
   ...
   // older VeraLux stretch script under src/scripts/VeraLux/).
...
function optFormatRecommendedRepositoriesText() {
   var text = "";
   text += "PI Workflow - Recommended Repositories\n";
   text += "================================================\n\n";
...
function optShowRecommendedRepositoriesDialog(parent) {
   ...
   title.text = "<b>PI Workflow Recommended Repositories</b>";
...
function optCloneView(view, baseId, showWindow) {
   ...
   var win = optCreateWindowLike(view, baseId || (view.id + "_clone"), channels, color);
...
function optCreateRgbFromChannels(viewR, viewG, viewB, baseId, metadataView) {
   ...
   var win = optCreateWindowLike(viewG, baseId || "Opt_RGB", 3, true);
   win.mainView.beginProcess(UndoFlag_NoSwapFile);
      ...
      },
      {
         ...
         check: function() {
            ...
            if (info.mode === "process")
               return optDependencyStatus("graxpert", "GraXpert", "Pre/Post", "ok", "Proceso nativo disponible.", "GraXpert esta disponible como proceso nativo de PixInsight; PI Workflow lo usa para Background Extraction y GraXpert Denoise.");
...
function optApplyPreCandidate(view, actionKey, dialog) {
   ...
   if (actionKey === "gradient") {
      ...
      if (hasComboGradient)
         optRequireDependentComboSelection(dialog.comboPreGradient, "Pre Gradient Correction");
   ...
   if (actionKey === "decon") {
      ...
      if (hasComboDecon)
         optRequireDependentComboSelection(dialog.comboPreDecon, "Pre Deconvolution");
...
function optGraXpertExecutableCandidatePaths() {
   ...
   var paths = [
      "C:/Users/ninoc/AppData/Local/Programs/GraXpert/GraXpert.exe",
      "C:/Users/ninoc/AppData/Local/Programs/GraXpert/graxpert.exe",
...
function optRunCosmicClarityCLI(args) {
   var isWin = (CoreApplication.platform === "MSWINDOWS" || CoreApplication.platform === "Windows");
   var candidates = [];
   ...
   for (var ci = 0; ci < candidates.length; ++ci) {
      ...
      while (proc.isStarting || proc.isRunning) processEvents();
...
function optRunCosmicClarityOnView(targetView, params) {
   ...
   try {
      ...
      var runResult = optRunCosmicClarityCLI(args);
...
function optEnforceMemoryBudget(dialog, selfDescription) {
   if (!dialog)
      return 0;
   var entries = optMemorySlotsCensus(dialog);
   var total = 0;
...
function optTrySetComboItemEnabled(combo, index, enabled) {
   if (!combo)
      return false;
   try {
      if (typeof combo.setItemEnabled === "function") {
         combo.setItemEnabled(index, enabled === true);
         return true;
      }
   } catch (e0) {}
   try {
      if (typeof combo.enableItem === "function") {
         combo.enableItem(index, enabled === true);
         return true;
      }
   } catch (e1) {}
   try {
      if (typeof combo.disableItem === "function") {
         combo.disableItem(index, enabled !== true);
         return true;
      }
   } catch (e2) {}
   return false;
...
function optTrySetComboItemText(combo, index, text) {
   try {
      if (combo && typeof combo.setItemText === "function") {
...
function optProcessAvailable(processNames, iconNames) {
   return optCreateGenericProcessInstance(processNames || [], iconNames || []) != null;
}

function optDependencyAvailableForMenu(depKey) {
   try {
      switch (depKey) {
      case "mgc":
         return optProcessAvailable(["MultiscaleGradientCorrection"], ["MGC"]);
      case "autodbe":
         return optIsAutoDBEAvailable();
      case "abe":
         return optProcessAvailable(["AutomaticBackgroundExtractor"], []);
      case "graxpert":
         return optGraXpertSupportMode() !== "";
      case "graxpert_denoise":
         return optHasGraXpertProcess();
      case "blurx":
         return typeof BlurXTerminator !== "undefined";
      case "cosmic_clarity":
         return optIsCosmicClarityAvailable();
      case "mas":
         return optProcessAvailable(["MultiscaleAdaptiveStretch"], []);
      case "veralux":
         return optVeraLuxAvailable();
      case "curves":
         return optProcessAvailable(["CurvesTransformation"], []);
      case "noisex":
         return optProcessAvailable(["NoiseXTerminator"], ["NXT", "NoiseXTerminator"]);
      case "tgvdenoise":
         return optProcessAvailable(["TGVDenoise"], []);
      case "unsharp_mask":
         return optProcessAvailable(["UnsharpMask"], []);
      case "hdrmt":
         return optProcessAvailable(["HDRMultiscaleTransform"], []);
      case "lhe":
         return optProcessAvailable(["LocalHistogramEqualization"], []);
      default:
         return true;
      }
   } catch (e) {
...
function optMenuDependencyMissingReason(spec) {
   if (!spec || !spec.depKey)
      return "";
   if (spec.missingReason && spec.missingReason.length > 0)
      return spec.missingReason;
   return "Required PixInsight process/script is not available in this running installation.";
}

function optEvaluateDependentMenuSpecs(specs) {
   var out = [];
   for (var i = 0; specs && i < specs.length; ++i) {
      var spec = specs[i] || {};
      var available = true;
      if (typeof spec.available === "function") {
         try { available = spec.available() === true; } catch (e0) { available = false; }
      } else if (spec.depKey) {
         available = optDependencyAvailableForMenu(spec.depKey);
      }
      out.push({
         label: spec.label || "",
         depKey: spec.depKey || "",
         available: available === true,
         missingReason: optMenuDependencyMissingReason(spec)
      });
   }
   return out;
}

function optFirstAvailableMenuIndex(states) {
   for (var i = 0; states && i < states.length; ++i)
      if (states[i].available === true)
         return i;
   return -1;
}

function optRefreshDependentCombo(combo) {
   if (!combo || !combo.__optDependentMenuSpecs)
      return;
   var states = optEvaluateDependentMenuSpecs(combo.__optDependentMenuSpecs);
   combo.__optDependentMenuStates = states;
   var nativeItemDisable = false;
   for (var i = 0; i < states.length; ++i) {
      nativeItemDisable = optTrySetComboItemEnabled(combo, i, states[i].available) || nativeItemDisable;
      var label = states[i].label;
      if (states[i].available !== true)
         label += " (not installed)";
      optTrySetComboItemText(combo, i, label);
   }
   combo.__optNativeItemDisable = nativeItemDisable === true;
   var first = optFirstAvailableMenuIndex(states);
   combo.enabled = first >= 0;
   var current = 0;
   try { current = combo.currentItem; } catch (e0) { current = 0; }
   if (first >= 0 && (!states[current] || states[current].available !== true)) {
      combo.__optSuppressDependentMenuCallback = true;
      try { combo.currentItem = first; } catch (e1) {}
      combo.__optSuppressDependentMenuCallback = false;
   }
}

function optInstallDependentCombo(combo, specs, onAllowedSelected) {
   if (!combo)
      return;
   combo.__optDependentMenuSpecs = specs || [];
   combo.__optDependentMenuCallback = onAllowedSelected;
   combo.__optLastAvailableIndex = 0;
   optRefreshDependentCombo(combo);
   try { combo.__optLastAvailableIndex = combo.currentItem; } catch (e0) {}
   combo.onItemSelected = function(idx) {
      if (this.__optSuppressDependentMenuCallback === true)
         return;
      optRefreshDependentCombo(this);
      var states = this.__optDependentMenuStates || [];
      if (!states[idx] || states[idx].available !== true) {
         var fallback = optFirstAvailableMenuIndex(states);
         if (fallback < 0)
            fallback = this.__optLastAvailableIndex || 0;
         this.__optSuppressDependentMenuCallback = true;
         try { this.currentItem = fallback; } catch (e1) {}
         this.__optSuppressDependentMenuCallback = false;
         try {
            var label = states[idx] ? states[idx].label : "Option";
            var reason = states[idx] ? states[idx].missingReason : "Unavailable option.";
            console.warningln("=> PI Workflow: '" + label + "' is disabled. " + reason);
         } catch (e2) {}
         if (typeof this.__optDependentMenuCallback === "function")
            this.__optDependentMenuCallback(fallback);
         return;
      }
      this.__optLastAvailableIndex = idx;
      if (typeof this.__optDependentMenuCallback === "function")
         this.__optDependentMenuCallback(idx);
   };
}

function optRequireDependentComboSelection(combo, context) {
   if (!combo || !combo.__optDependentMenuSpecs)
      return true;
   optRefreshDependentCombo(combo);
   var idx = 0;
   try { idx = combo.currentItem; } catch (e0) { idx = 0; }
   var states = combo.__optDependentMenuStates || [];
   if (states[idx] && states[idx].available === true)
      return true;
   var label = states[idx] ? states[idx].label : (context || "Selected option");
   var reason = states[idx] ? states[idx].missingReason : "No available option is selected.";
   throw new Error((context || "Selected option") + " is disabled: " + label + ". " + reason);
}
...
OptSelectionPanel.prototype.addCombo = function(parentSizer, label, key, requireColor) {
...
OptPreviewPane.prototype.beginCandidate = function(stageName, transformFn, actionKey) {
   ...
   var candidate = optCloneView(this.currentView, "Opt_Candidate_" + this.currentKey + "_" + stageName, false);
...
OptPreviewPane.prototype.setToCurrent = function() {
   ...
   if (typeof this.__candidateUpgrader === "function") {
      ...
      try {
         ...
         if (optSafeView(upgraded) && upgraded.id !== this.candidateView.id) {
      ...
      if (!optSafeView(this.candidateView))
         return;
   }
   optCloseViews([this.currentGradientView]);
...
OptPreviewPane.prototype.exportCurrent = function() {
   ...
   if (!optSafeView(view))
      return;
   var exported = optCloneView(view, "Opt_Export_" + (this.currentKey || view.id), true);
...
OptPreviewPane.prototype.storeMemory = function(index) {
   ...
   if (!meta)
      meta = optBuildMemoryMeta(this, this.pendingStage || "Current", this.pendingActionKey || "", null);
   if (optSafeView(view)) {
      this.memory.store(index, this.currentKey || view.id, view, meta, gradientView);
      // #5: enforce memory budget after store; oldest non-protected slots evict first.
...
OptWorkflowTab.prototype.wireSelection = function() {
   var tab = this;
   this.selection.btnCombineMono.onClick = function() { tab.combineMono(); };
   this.selection.btnSeparateMono.onClick = function() { tab.processSeparateMono(); };
   this.selection.btnCombineNb.onClick = function() { tab.combineNb(); };
   this.selection.btnSeparateNb.onClick = function() { tab.processSeparateNb(); };
   this.selection.btnProcessRgb.onClick = function() { tab.processRgb(); };
};
...
OptWorkflowTab.prototype.combineMono = function() {
   ...
   var combined = optCreateRgbFromChannels(r, g, b, "R_G_B", g);
...
function optStretchParamsFromZone(zone) {
   ...
   return {
      ...
      vlx_bg: optNumericValue(zone.vlxBg, 0.10),
...
function OptStretchingEngine() {
   ...
   function masPixelLuminance(img, x, y, isRGB) {
      if (!isRGB)
         return img.sample(x, y, 0);
      return medianOfThree(img.sample(x, y, 0), img.sample(x, y, 1), img.sample(x, y, 2));
   }
   ...
   function buildMasLuminanceIntegral(view) {
      ...
      for (var y = 1; y <= h; ++y) {
         ...
         for (var x = 1; x <= w; ++x) {
            rowAccum += masPixelLuminance(img, x - 1, y - 1, isRGB);
   ...
   function computeMasRoiRobustStats(view, x0, y0, roiW, roiH) {
      ...
      for (var y = y0; y < y0 + roiH; ++y)
         for (var x = x0; x < x0 + roiW; ++x)
            values[n++] = masPixelLuminance(img, x, y, isRGB);
   ...
   this.runStretch = function(view, algoId, params) {
      ...
      if (algoId === "STF") {
         ...
         // script and the VeraLux stretch script.
...
function optApplyStretchCandidate(view, algoId, zone, dialog) {
   ...
   if (zone && zone.combo)
      optRequireDependentComboSelection(zone.combo, "Stretch algorithm");
...
function optBuildStretchZone(tab, title, isStars) {
   ...
   zone.scheduleCurvesLive = function(delayMs) {
      ...
      dlg.previewScheduler.request("stretch.curves." + (isStars ? "stars" : "rgb"), function() {
         ...
         if (!tab.preview.activate(key, false))
            return;
         tab.preview.beginCandidate("Stretch CURVES", function(candidate) {
            return optApplyStretchCandidate(candidate, "CURVES", zone, dlg);
         }, "stretch_curves");
      }, {
   ...
   var stretchMenuSpecs = [];
   for (var si = 0; si < algoLabels.length; ++si) {
      var sid = algoIds[si];
      stretchMenuSpecs.push({
         label: algoLabels[si],
         depKey: sid === "MAS" ? "mas" : (sid === "VLX" ? "veralux" : (sid === "CURVES" ? "curves" : "")),
         missingReason: sid === "MAS" ? "MultiscaleAdaptiveStretch is not available." :
            (sid === "VLX" ? "VeraLux is not available as a native process or loadable script." :
            (sid === "CURVES" ? "CurvesTransformation is not available." : ""))
      });
   }
   optInstallDependentCombo(zone.combo, stretchMenuSpecs, function() { zone.sync(); });
   zone.sync();
...
PIWorkflowOptDialog.prototype.configurePreTab = function() {
   ...
   this.preTab.addProcessSection("Gradient Correction", [{
      ...
      build: function(body) {
         ...
         optInstallDependentCombo(dlg.comboPreGradient, [
            { label: "MultiscaleGradient Correction (MGC)", depKey: "mgc", missingReason: "MultiscaleGradientCorrection is not available." },
            { label: "AutoDBE (SetiAstro)", depKey: "autodbe", missingReason: "AutoDBE could not be loaded from the running PixInsight script tree." },
            { label: "AutomaticBackgroundExtractor (ABE)", depKey: "abe", missingReason: "AutomaticBackgroundExtractor is not available." },
            { label: "GraXpert", depKey: "graxpert", missingReason: "GraXpert is not available as a native process or loadable script." }
         ], function(idx) { dlg.syncPreGradientPanels(idx); });
         dlg.syncPreGradientPanels(dlg.comboPreGradient.currentItem);
      }
   });
   ...
   this.preTab.addProcessSection("Color Calibration", [
      ...
      { text: "SPCC", stage: "Color Calibration (SPCC)", actionKey: "spcc", width: 80 },
   ...
   this.preTab.addProcessSection("Deconvolution", [{
      ...
      build: function(body) {
         ...
         optInstallDependentCombo(dlg.comboPreDecon, [
            { label: "BlurXTerminator", depKey: "blurx", missingReason: "BlurXTerminator is not available." },
            { label: "Cosmic Clarity (SetiAstro)", depKey: "cosmic_clarity", missingReason: "ExternalProcess is not available, so Cosmic Clarity cannot be launched." }
         ], function(idx) { dlg.syncPreDeconPanels(idx); });
         dlg.syncPreDeconPanels(dlg.comboPreDecon.currentItem);
      }
   });
...
function optRequireActivePostMaskView(dialog, workView) {
   if (!dialog || !optSafeView(dialog.postActiveMask))
      throw new Error("No active Post mask is available. Generate a mask first.");
   if (!optSafeView(workView))
      throw new Error("There is no valid target view for the active mask.");
   var maskView = dialog.postActiveMask;
   if (workView.image.width !== maskView.image.width || workView.image.height !== maskView.image.height)
      throw new Error("The active mask geometry does not match the target image.");
   try {
      if (workView.window && maskView.window && typeof workView.window.isMaskCompatible === "function" &&
          !workView.window.isMaskCompatible(maskView.window))
         throw new Error("The active mask is not compatible with the target image.");
   } catch (eCompat) {
      if (eCompat && eCompat.message && eCompat.message.indexOf("not compatible") >= 0)
         throw eCompat;
   }
   return maskView;
}

function optBlendProcessedViewWithMask(targetView, originalView, maskView) {
   if (!optSafeView(targetView) || !optSafeView(originalView) || !optSafeView(maskView))
      throw new Error("Masked blending requires valid target, original, and mask views.");
   if (targetView.image.width !== originalView.image.width ||
       targetView.image.height !== originalView.image.height ||
       targetView.image.numberOfChannels !== originalView.image.numberOfChannels)
      throw new Error("Masked blending requires identical original and processed image geometry.");
   if (targetView.image.width !== maskView.image.width || targetView.image.height !== maskView.image.height)
      throw new Error("The active mask geometry does not match the processed image.");
   var pm = new PixelMath();
   pm.createNewImage = false;
   pm.showNewImage = false;
   var maskId = maskView.id;
   var baseId = originalView.id;
   if (targetView.image.numberOfChannels >= 3) {
      pm.useSingleExpression = false;
      pm.expression  = "($T[0]*" + maskId + ")+(" + baseId + "[0]*(1-" + maskId + "))";
      pm.expression1 = "($T[1]*" + maskId + ")+(" + baseId + "[1]*(1-" + maskId + "))";
      pm.expression2 = "($T[2]*" + maskId + ")+(" + baseId + "[2]*(1-" + maskId + "))";
   } else {
      pm.useSingleExpression = true;
      pm.expression = "($T*" + maskId + ")+(" + baseId + "*(1-" + maskId + "))";
   }
   pm.executeOn(targetView);
   return targetView;
}
...
function optRunPostOperationWithOptionalMask(workView, dialog, useMask, operationFn) {
   ...
   try {
      if (useMask === true) {
         optRequireActivePostMaskView(dialog, workView);
         originalView = optCloneView(workView, "Opt_PostMaskBase", false);
         if (!optSafeView(originalView))
            throw new Error("Could not create the temporary base image required for masked processing.");
      }
      var result = operationFn(workView);
      if (useMask === true) {
         var maskView = optRequireActivePostMaskView(dialog, workView);
         optBlendProcessedViewWithMask(workView, originalView, maskView);
      }
      return result || workView;
   } finally {
      optCloseView(originalView);
   }
}
...
function optPostFameFillPolygon(outImg, srcImg, polygon, pixelFn) {
   ...
   for (var y = Math.max(0, Math.floor(minY)); y <= Math.min(h - 1, Math.ceil(maxY)); ++y) {
      ...
      for (var k = 0; k + 1 < xs.length; k += 2) {
         for (var x = Math.max(0, xs[k]); x <= Math.min(w - 1, xs[k + 1]); ++x) {
            ...
            if (v > outImg.sample(x, y, 0)) outImg.setSample(v, x, y, 0);
         }
      }
   }
}

function optPostFameRasterizeCircle(outImg, srcImg, cx, cy, radius, pixelFn) {
...
function optPostFameFillBrush(outImg, srcImg, shape, pixelFn) {
   var centers = shape.centers || [], radius = Math.max(1, shape.radius || 10);
   for (var i = 0; i < centers.length; ++i)
      optPostFameRasterizeCircle(outImg, srcImg, centers[i][0], centers[i][1], radius, pixelFn);
   for (var s = 0; s < centers.length - 1; ++s) {
      ...
      for (var t = 0; t <= steps; ++t) {
         var u = t / steps;
         optPostFameRasterizeCircle(outImg, srcImg, p0[0] + (p1[0] - p0[0]) * u, p0[1] + (p1[1] - p0[1]) * u, radius, pixelFn);
      }
   }
}

function optPostFameFillSpray(outImg, srcImg, shape, pixelFn) {
...
function optBuildPostFameMaskImage(tv, dialog) {
   ...
   for (var i = 0; i < st.shapes.length; ++i) {
      ...
      if (shape.type === "Brush")      optPostFameFillBrush(outImg, srcImg, shape, pixelFn);
      else if (shape.type === "SprayCan") optPostFameFillSpray(outImg, srcImg, shape, pixelFn);
      else                             optPostFameFillPolygon(outImg, srcImg, optPostFameGetShapePoints(shape), pixelFn);
   }
   return outImg;
}
...
function optApplySmoothToMaskArr(maskArr, W, H, sigma) {
   ...
   for (var y = 0; y < H; ++y) {
      ...
      for (var x = 0; x < W; ++x) {
         ...
         for (var k = 0; k < kernel.length; ++k) {
            var xi = Math.max(0, Math.min(W - 1, x + k - radius));
            s += maskArr[rowOff + xi] * kernel[k];
         }
         tmp[rowOff + x] = s;
      }
   }
   for (var x2 = 0; x2 < W; ++x2) {
      for (var y2 = 0; y2 < H; ++y2) {
         var s2 = 0;
         for (var k2 = 0; k2 < kernel.length; ++k2) {
            var yi = Math.max(0, Math.min(H - 1, y2 + k2 - radius));
            s2 += tmp[yi * W + x2] * kernel[k2];
         }
         maskArr[y2 * W + x2] = s2;
      }
   }
...
function optBuildMaskMemoryPanel(dialog, parent, previewPane) {
   ...
   for (var i = 0; i < OPT_MASK_MEMORY_SLOTS; ++i) {
      ...
      b.onClick = function() {
         ...
         // #5: enforce memory budget after mask memory store.
         try { optEnforceMemoryBudget(dialog, "mask memory store"); } catch (eMB) {}
         if (typeof dialog.refreshPostMaskMemoryUi === "function")
            dialog.refreshPostMaskMemoryUi();
      };
   ...
   var btnSet = optPrimaryButton(row, "Set to Active Mask", 145);
   dialog.btnPostShowHideMask = btnShowHide;
   dialog.btnPostSetActiveMask = btnSet;
   dialog.refreshPostMaskMemoryUi = function() {
      if (dialog.btnPostShowHideMask)
         dialog.btnPostShowHideMask.enabled = optSafeView(dialog.postActiveMask);
...
function optApplyPostCandidate(view, actionKey, dialog) {
   ...
   if (actionKey === "post_nr") {
      return optRunPostOperationWithOptionalMask(view, dialog, optChecked(dialog.chkPostNRUseMask, false), function(targetView) {
         if (dialog && dialog.comboPostNR)
            optRequireDependentComboSelection(dialog.comboPostNR, "Post Noise Reduction");
   ...
   if (actionKey === "post_sharp") {
      return optRunPostOperationWithOptionalMask(view, dialog, optChecked(dialog.chkPostSharpUseMask, false), function(targetView) {
         if (dialog && dialog.comboPostSharp)
            optRequireDependentComboSelection(dialog.comboPostSharp, "Post Sharpening");
...
function optSchedulePostLiveCandidate(dialog, key, stageName, actionKey, delayMs) {
   ...
   dialog.previewScheduler.request(key, function() {
      dialog.postTab.preview.beginCandidate(stageName, function(candidate) {
         return optApplyPostCandidate(candidate, actionKey, dialog);
      }, actionKey);
   }, {
...
function optPrepareCcSlotView(dialog, slot) {
   var key = optCcSlotSourceKey(slot);
   if (!key)
      return null;
   var rec = dialog.store.record(key);
   ...
   try {
      ...
      if (optSafeView(slotMask)) {
         if (work.image.width !== slotMask.image.width ||
         ...
         work.window.mask = slotMask.window;
...
function optGetCachedCcSlot(dialog, slot, live, liveMaxDim) {
   ...
   slot[cacheProp] = null;
   var prepared = optPrepareCcSlotView(dialog, slot);
   if (!optSafeView(prepared))
      return null;
   if (live === true)
      optDownsamplePreparedView(prepared, liveMaxDim || OPT_CC_LIVE_MAX_DIM);
...
function optBuildPostNoiseSection(dlg) {
   dlg.postTab.addProcessSection("Noise Reduction", [{
      ...
      build: function(body) {
         ...
         optInstallDependentCombo(dlg.comboPostNR, [
            { label: "NoiseXTerminator", depKey: "noisex", missingReason: "NoiseXTerminator is not available." },
            { label: "TGVDenoise", depKey: "tgvdenoise", missingReason: "TGVDenoise is not available." },
            { label: "Cosmic Clarity (Seti Astro)", depKey: "cosmic_clarity", missingReason: "ExternalProcess is not available, so Cosmic Clarity cannot be launched." },
            { label: "GraXpert Denoise", depKey: "graxpert_denoise", missingReason: "GraXpert Denoise requires the native GraXpert process." }
         ], function(idx) { dlg.syncPostNRPanels(idx); });
         dlg.syncPostNRPanels(dlg.comboPostNR.currentItem);
      }
   });
...
function optBuildPostSharpeningSection(dlg) {
   dlg.postTab.addProcessSection("Sharpening", [{
      ...
      build: function(body) {
         ...
         optInstallDependentCombo(dlg.comboPostSharp, [
            { label: "BlurXTerminator", depKey: "blurx", missingReason: "BlurXTerminator is not available." },
            { label: "Unsharp Mask", depKey: "unsharp_mask", missingReason: "UnsharpMask is not available." },
            { label: "HDR Multiscale Transform", depKey: "hdrmt", missingReason: "HDRMultiscaleTransform is not available." },
            { label: "Local Histogram Equalization", depKey: "lhe", missingReason: "LocalHistogramEqualization is not available." },
            { label: "Dark Structure Enhance" },
            { label: "Cosmic Clarity", depKey: "cosmic_clarity", missingReason: "ExternalProcess is not available, so Cosmic Clarity cannot be launched." }
         ], function(idx) { dlg.syncPostSharpPanels(idx); });
         dlg.syncPostSharpPanels(dlg.comboPostSharp.currentItem);
      }
   });
...
PIWorkflowOptDialog.prototype.configureCcTab = function() {
   ...
   dlg.scheduleCcSlotsPreview = function(delayMs) {
      dlg.previewScheduler.request("cc.slots", function() {
         ...
         dlg.ccTab.preview.beginCandidateFromFactory("Image" + slot.index, function() {
...
PIWorkflowOptDialog.prototype.runDependencyChecks = function() {
   ...
   if (typeof this.refreshDependencyMenus === "function")
      this.refreshDependencyMenus();
   return this.dependencyReport;
};

PIWorkflowOptDialog.prototype.refreshDependencyMenus = function() {
   function refreshCombo(combo, syncFn) {
      if (!combo)
         return;
      optRefreshDependentCombo(combo);
   ...
   var dlg = this;
   refreshCombo(this.comboPreGradient, function(idx) { if (dlg.syncPreGradientPanels) dlg.syncPreGradientPanels(idx); });
   refreshCombo(this.comboPreDecon, function(idx) { if (dlg.syncPreDeconPanels) dlg.syncPreDeconPanels(idx); });
   if (this.stretchZoneRgb)
      refreshCombo(this.stretchZoneRgb.combo, function() { dlg.stretchZoneRgb.sync(); });
   if (this.stretchZoneStars)
      refreshCombo(this.stretchZoneStars.combo, function() { dlg.stretchZoneStars.sync(); });
   refreshCombo(this.comboPostNR, function(idx) { if (dlg.syncPostNRPanels) dlg.syncPostNRPanels(idx); });
   refreshCombo(this.comboPostSharp, function(idx) { if (dlg.syncPostSharpPanels) dlg.syncPostSharpPanels(idx); });
};
...
PIWorkflowOptDialog.prototype.finalCleanup = function() {
   ...
   this.preTab.preview.releaseTransient();
   this.stretchTab.preview.releaseTransient();
   this.postTab.preview.releaseTransient();
   this.ccTab.preview.releaseTransient();
...
function optMain() {
   ...
   var dlg = new PIWorkflowOptDialog();
   dlg.execute();
